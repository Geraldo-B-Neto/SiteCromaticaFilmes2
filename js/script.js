...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const equipe = [
        {
            ...
            frase: 'A música é a linguagem universal que conecta corações e mentes.',
            descricao: 'Com mais de 15 anos de experiência em produção musical, João é responsável por liderar nossa equipe de instrutores e desenvolver o currículo acadêmico.'
        },
        {
            nome: 'Maria Santos',
            cargo: 'Coordenadora de Produção Audiovisual',
            frase: 'Cada frame conta uma história, cada plano é uma emoção.',
            descricao: 'Especialista em direção de fotografia e edição, Maria traz sua experiência do cinema para nossas salas de aula.'
        },
        {
            nome: 'Carlos Oliveira',
            cargo: 'Professor de Educação Criativa',
            frase: 'A criatividade é a inteligência se divertindo.',
            descricao: 'Com formação em artes visuais e educação, Carlos desenvolve metodologias inovadoras para estimular a criatividade dos alunos.'
        }
    ];
        ...
        equipe.forEach(membro => {
            const membroHTML = `
                <div class="membro-equipe">
                    <div class="foto">
                        <img src="${membro.imagem || 'assets/imgs/equipe/placeholder.jpg'}" alt="${membro.nome}">
                    </div>
                    <p class="frase">${membro.frase}</p>
                    <h3>${membro.nome}</h3>
                    <span class="cargo">${membro.cargo}</span>
                    <p>${membro.descricao}</p>
                </div>
            `;
            equipeGrid.innerHTML += membroHTML;
        });
    }
    
    // Portfólio agora é definido diretamente no HTML dentro de .portfolio-grid
    ...
    if (depoimentosSlider) {
        ...
        // Inicializar slider de depoimentos (simples para demonstração)
        // Em um projeto real, você pode usar Slick, Swiper ou outra biblioteca
        ...
        if (depoimentoItems.length > 0) {
            ...
            // Trocar depoimentos a cada 5 segundos
            setInterval(() => {
    ...
    if (contatoForm) {
        contatoForm.addEventListener('submit', function(e) {
            ...
            const isExternal = action.includes('formsubmit.co');
            if (isExternal) {
                // Permite o envio normal ao FormSubmit
                return;
            }

            // Caso não haja action externa, mantém o comportamento de simulação
            e.preventDefault();
            const formData = new FormData(this);
            const formObject = {};
            formData.forEach((value, key) => { formObject[key] = value; });
            console.log('Formulário enviado (simulado):', formObject);
